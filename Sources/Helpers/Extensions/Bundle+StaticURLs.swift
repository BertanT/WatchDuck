...
extension Bundle {
    func staticURLs(forResourcesWithExtension ext: StaticString?, subdirectory: StaticString? = nil, localization: StaticString? = nil) -> [URL] {
        guard let urls = self.urls(
            forResourcesWithExtension: ext?.description,
            subdirectory: subdirectory?.description,
            localization: localization?.description
        ) else {
            fatalError(
                """
                Invalid static URL array query for bundle resources. (matching extension: \"\(String(describing: ext))\",\
